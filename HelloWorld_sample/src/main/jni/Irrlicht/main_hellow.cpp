...
static void handle_cmd(struct android_app* app, int32_t cmd)
{
    ...
    switch (cmd)
    {
        ...
        data->device = createDevice( video::EDT_SOFTWARE,
                    ...
                    LOGI("marky  createDevice -> data->device = %X ",  data->device);
...
int main()
{
	...
	*/
	IAnimatedMesh* mesh = smgr->getMesh("/sdcard/irr/sydney.md2");
	if (!mesh)
	{
		device->drop();
		return 1;
	...
	*/
	if (node)
	{
		...
		node->setMaterialTexture( 0, driver->getTexture("/sdcard/irr/sydney.bmp") );
	}
