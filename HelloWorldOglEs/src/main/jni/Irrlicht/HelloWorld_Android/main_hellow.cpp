        ...
        case APP_CMD_INIT_WINDOW:
			{
                ...
                //	param.DriverType = EDT_OGLES1;				// android:glEsVersion in AndroidManifest.xml should be "0x00010000" (requesting 0x00020000 will also guarantee that ES1 works)
                	...
                	data->device = createDeviceEx(param);
                	receiver.Init(data->device);
                }
                */
				data->driver = data->device->getVideoDriver();
				data->smgr = data->device->getSceneManager();
				data->mesh = data->smgr->getMesh("/sdcard/irr/sydney.md2");
				if (!data->mesh)
				{
					data->device->drop();
					return;
				...
				if (node)
				{
					...
					node->setMaterialTexture( 0, data->driver->getTexture("/sdcard/irr/sydney.bmp") );
				}
...
static void handle_cmd(struct android_app* app, int32_t cmd)
{
    ...
    switch (cmd)
    {
        ...
        data->device = createDevice( video::EDT_OGLES2,
					...
					data->images = data->driver->getTexture("/sdcard/irr/2ddemo.png");
					if (!data->images) {
					...
					data->font2 = data->device->getGUIEnvironment()->getBuiltInFont();//getFont("/sdcard/irr/fonthaettenschweiler.bmp");
#endif
                    ...
                    if (data->mesh) {
				        ...
				        if (data->node)
				        {
					        ...
					        data->node->setMaterialTexture( 0, data->driver->getTexture("/sdcard/irr/sydney.bmp") );
