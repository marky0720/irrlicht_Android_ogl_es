...
namespace irr
{
			...
			bool stencilbuffer, bool vsync, IEventReceiver* res)
	{
		...
		p.Vsync = vsync;
		p.EventReceiver = res;
        LOGI("marky IRRCALLCONV createDevice 0");
	...
	extern "C" IRRLICHT_API IrrlichtDevice* IRRCALLCONV createDeviceEx(const SIrrlichtCreationParameters& params)
	{
        ...
        LOGI("marky createDeviceEx 0");
#ifdef _IRR_COMPILE_WITH_WINDOWS_DEVICE_
        LOGI("marky createDeviceEx 1");
		if (params.DeviceType == EIDT_WIN32 || (!dev && params.DeviceType == EIDT_BEST))
			dev = new CIrrDeviceWin32(params);
#endif

#ifdef _IRR_COMPILE_WITH_OSX_DEVICE_
        LOGI("marky createDeviceEx 2");
		if (params.DeviceType == EIDT_OSX || (!dev && params.DeviceType == EIDT_BEST))
			dev = new CIrrDeviceMacOSX(params);
#endif

#ifdef _IRR_COMPILE_WITH_WINDOWS_CE_DEVICE_
        LOGI("marky createDeviceEx 3");
...
#ifdef _IRR_COMPILE_WITH_IPHONE_DEVICE_
        LOGI("marky createDeviceEx 4");
		if (params.DeviceType == EIDT_IPHONE || (!dev && params.DeviceType == EIDT_BEST))
			dev = new CIrrDeviceIPhone(params);
#endif

#ifdef _IRR_COMPILE_WITH_ANDROID_DEVICE_
        LOGI("marky createDeviceEx 5, params.DeviceType=%x, %x", params.DeviceType, EIDT_ANDROID);
...
#ifdef _IRR_COMPILE_WITH_FB_DEVICE_
        LOGI("marky createDeviceEx 6");
		if (params.DeviceType == EIDT_FRAMEBUFFER || (!dev && params.DeviceType == EIDT_BEST))
			dev = new CIrrDeviceFB(params);
#endif

#ifdef _IRR_COMPILE_WITH_CONSOLE_DEVICE_
       LOGI("marky createDeviceEx 7");
		if (params.DeviceType == EIDT_CONSOLE || (!dev && params.DeviceType == EIDT_BEST))
			dev = new CIrrDeviceConsole(params);
#endif
       LOGI("marky createDeviceEx 8 %X. %X, %X, %X", dev, dev->getVideoDriver(), params.DriverType, video::EDT_NULL);
       //modify by marky
        return dev;
		if (dev && !dev->getVideoDriver() && params.DriverType != video::EDT_NULL)
		{
			LOGI("marky createDeviceEx 8-1");
			dev->closeDevice(); // destroy window
			dev->run(); // consume quit message
			dev->drop();
			dev = 0;
		}
        LOGI("marky createDeviceEx 9");
		return dev;
	}
