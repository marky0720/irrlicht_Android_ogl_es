	...
	: CIrrDeviceStub(param), Focused(false), Initialized(false), Paused(true)
{
	...
	LOGI("marky CIrrDeviceAndroid 1");

	SensorManager = ASensorManager_getInstance();

	LOGI("marky CIrrDeviceAndroid 2.");
	SensorEventQueue = ASensorManager_createEventQueue(SensorManager, Android->looper, LOOPER_ID_USER, 0, 0);

	LOGI("marky CIrrDeviceAndroid 3");

	Android->onInputEvent = handleInput;
    LOGI("marky CIrrDeviceAndroid 4.");

	// Create EGL manager.
	ContextManager = new video::CEGLManager();
    LOGI("marky CIrrDeviceAndroid 5.");
	LOGI("Waiting for Android activity window to be created.", ELL_DEBUG);
